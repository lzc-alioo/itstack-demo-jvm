...
20220420更新
解决子上一个版本中误将操作数当成了指令，识别成了unknow，本版本中已剔除
```
Classfile       : /Users/mac/work/gitstudy/itstack-demo-jvm/itstack-lzc/target/test-classes/com/lzc/wuxin/demo/HelloWorld.class
  Last modified : 2022-04-17; size 711 bytes
  MD5 checksum  : 17e695fb34c8c10f716ac6e92c8609f1
...
20220421
发现如果要记录打印指令执行时用到的操作数（操作数包括了常量、方法、变量），需要去修改每个操作指令类，并且需要在操作指令类中进行修改，没有啥特别好的办法

20220422
在Instruction中增加默认实现方法debug，打印操作指令，如果需要打印操作数内容，以及操作指令需要用到的方法引用或者属性引用
 ...
 #39 = Utf8                (B)Ljava/lang/Byte;           
{
  ...
  static {};
    ...
    Code: 
      stack=1, locals=0, args_size=0
        bipush              -15       
        invoke_static       #2        // Method java/lang/Byte.valueOf:(B)Ljava/lang/Byte;
        put_static          #3        // Field abc2:Ljava/lang/Byte;
        return                        
      LineNumberTable:
        line 6: 0
      LocalVariableTable:
