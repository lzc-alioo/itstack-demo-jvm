...
public class DateTimeUtil {
    ...
    /**
     * 转换日期格式
     * 例如：入参：(2022-01-11, yyyy-MM-dd, yyyyMMdd)
     *      出参：20220111
     *
     * @param param        日期参数
     * @param sourceFormat 输入日期参数类型
     * @param targetFormat 输出日期参数类型
     * @return 输出日期
     */
    public static String transferDateFromParam(String param, String sourceFormat, String targetFormat) {
        if (StringUtils.isBlank(param) || StringUtils.isBlank(sourceFormat) || StringUtils.isBlank(targetFormat)) {
            return StringUtils.EMPTY;
        }
        try {
            Date date = toDateFromStr(param, sourceFormat);
            return Objects.isNull(date) ? StringUtils.EMPTY : toDateTimeString(date, targetFormat);
        } catch (Exception e) {
            return StringUtils.EMPTY;
        }
    }
    ...
    /**
     * 指定格式的日期转换，将 yyyyMMdd 转换为 yyyy-MM-dd
     * 例如：2022-01-11 转换为 20220111
     *
     * @param param 日期参数（yyyyMMdd）
     * @return 转换后的日期（yyyy-MM-dd）
     */
    public static String transferDateToYyyyMMdd(Integer param) {
        if (Objects.isNull(param)) {
            return StringUtils.EMPTY;
        }
        return transferDateFromParam(param.toString(), "yyyyMMdd", "yyyy-MM-dd");
    }
}
